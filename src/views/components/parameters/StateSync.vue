  ...
  <b-row>
    ...
    <b-col md="12">
      <b-card border-variant="primary">
        ...
        The Tendermint Core 0.34 release includes support for state sync, which allows a new node to join a network by
        fetching a snapshot of the application state at a recent height instead of fetching and replaying all
        historical
        blocks. This can reduce the time needed to sync with the network from days to minutes.
        Click <a href="https://blog.cosmos.network/cosmos-sdk-state-sync-guide-99e4cf43be2f">here</a> for more
        infomation.
      </b-card>
    </b-col>
    <b-col md="6">
      <b-card>
        ...
        <b-form-textarea id="snapshot" v-model="snapshot" readonly rows="8" class="mt-1" />
      </b-card>
    </b-col>
    <b-col md="6">
      <b-card>
        ...
        <b-form-textarea id="textarea-state" v-model="state" :state="valid" readonly placeholder="Loading..." rows="7"
          class="my-1" @change="check()" />
        <b class="mt-1">3. Start the daemon: <span v-if="daemon"><code>{{ daemon }} start</code></span></b><br>
        If you are resetting node, run <code>{{ daemon }} unsafe-reset-all</code> or <code>{{ daemon }} tendermint
