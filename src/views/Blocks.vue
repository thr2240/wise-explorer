<template>
  <div>
    <b-card no-body class="text-truncate" border-variant="primary">
      <b-tabs pills class="mt-1">
        ...
        <b-tab active>
          ...
          <b-table :items="blocks" :fields="list_fields" foot-variant="light" head-variant='light' :sort-desc="true"
            ...
            <template #cell(hash)="data">
              <small>
                {{ shortHash(data.item.block_id.hash) }}
              </small>
            </template>
            ...
            <template #cell(txs)="data">
              <b-badge v-b-tooltip v-b-tooltip.hover.v-dark :title="`${data.item.block.data.txs.length} transactions`"
              ...
              <b-badge v-for="(tx, i) in initData(data.item.block, )" variant="none" id="transaction" :key="i">
                <b-avatar-group>
                  <b-avatar v-for="(msg, j) in tx.messages" :key="j" id="message" :size="42"
                    :variant="`light-${msgColor(msg)[0]}`" :badge="`${i+1}`" badge-top badge-right
                    :badge-variant="`${msgColor(msg)[0]}`" v-b-tooltip :title="`${msg.typeUrl}`"
            ...
            <template #cell(time)="data">
              <b-badge variant="light-info" size="sm">
        ...
        <b-tab title="Transactions">
          <b-table :items="txs" :fields="txFields" striped borderless sticky-header stacked="sm" hover responsive>
            <template #cell(fee)="data">
              <b-badge v-if="data.item.fee.length > 0" variant="light-warning">{{ data.item.fee[0].amount }} {{
              data.item.fee[0].denom }}</b-badge>
              <small v-else class="text-muted">No fee</small>
            </template>
            <template #cell(memo)="data">
              <small v-if="data.item.memo" class="text-white">{{ data.item.memo }}</small>
              <small v-else class="text-muted">No memo</small>
            </template>
            <template #cell(hash)="data">
            ...
            <template #cell(status)="data">
              <b-badge v-if="data.item.code===0" variant="light-success">
                Success
              </b-badge>
            ...
            <template #cell(messages)="data">
              <b-avatar-group>
                <b-avatar v-for="(msg, j) in data.item.messages" :key="j" id="message" :size="42"
                  :variant="`light-${msgColor(msg)[0]}`" v-b-tooltip :title="`${msg.typeUrl}`" v-b-tooltip.hover.v-dark>

                  <b-icon-cash-stack v-if="msg.typeUrl == '/cosmos.distribution.v1beta1.MsgWithdrawDelegatorReward'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/cosmos.authz.v1beta1.MsgExec'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/cosmos.authz.v1beta1.MsgRevoke'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/cosmos.authz.v1beta1.MsgGrant'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/cosmos.gov.v1beta1.MsgDeposit'" />
                  ...
                  <b-icon-play v-else-if="msg.typeUrl == '/ollo.loan.MsgRequestLoan'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/ollo.loan.MsgApproveLoan'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/ollo.loan.MsgRepayLoan'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/ollo.loan.MsgLiquidateLoan'" />
                  <b-icon-play v-else-if="msg.typeUrl == '/ollo.loan.MsgCancelLoan'" />
  ...
  data() {
    return {
      ...
      txFields: [
        ...
        { key: 'fee', sortable: true, formatter: v => tokenFormatter(v) },
