<template>
  <div>
    <div v-if="!error">
      <b-card border-variant="primary" class="d-flex flex-row">
        <b-card-header class="pt-0 pl-0 pr-0">
          <b-card-title>
            <div class="d-flex flex-row align-items-center text-truncate">
              <b-avatar id="address-qr" rounded size="42">
                <feather-icon icon="CameraIcon" size="20" />
              </b-avatar>
              &nbsp;&nbsp;Assets
              <div class="ml-2">
                <small class="text-muted">{{ address }}</small>&nbsp;
                <b-badge style="cursor: pointer;" variant="light-primary" @click="copy()">
          ...
          <div>
            <b-button v-b-modal.operation-modal variant="primary" size="sm" class="mr-25"
              @click="setOperationModalType('Transfer')">
              <feather-icon icon="SendIcon" class="d-md-none" /><small class="d-none d-md-block">Transfer</small>
            </b-button>
            <b-button v-b-modal.operation-modal variant="danger" size="sm"
        ...
        <b-card-body class="pl-0 pr-0">
          <b-row>
            <b-col xm="12" md="4">
              <chart-component-doughnut v-if="chartData" :height="235" :width="235" :data="chartData" class="mb-3" />
            </b-col>
            <b-col xm="12" md="5">
              <!-- tokens -->
              <div v-for="(token, index) in assetTable.items" :key="`asset-${index}`"
                ...
                <div class="d-flex align-items-center">
                  ...
                  <b-badge :variant="`${token.color}`" class="ml-1">{{ token.percent }}%</b-badge>
                  <span style="font-weight: 700;" class="text-capitalize font-weight-bold ml-75 d-none d-md-block">{{
                  token.type }}
                  </span>
                </div>
                <div class="d-flex flex-column">
                  <b-badge variant="light-primary" v-b-tooltip.hover.top="token.denom" v-b-tooltip.hover.v-dark
                    class="text-right">{{
                    formatToken(token) }}</b-badge>
                  <small class="text-muted text-right">{{ currency }}{{ formatNumber(token.currency) }}</small>
                </div>
              </div>
              <!--/ tokens -->
              <div class="text-right" style="padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05);">
                <small>Total:</small>&nbsp;&nbsp;
                <b-badge size="lg" style="font-size: 14px;" variant="light-primary">{{ currency }}{{
                formatNumber(assetTable.currency) }}
                </b-badge>
              </div>
            </b-col>
          </b-row>
      ...
      <b-card v-if="unbonding && unbonding.length > 0">
        ...
        <b-card-body class="pl-0 pr-0">
          <b-row v-for="item in unbonding" :key="item.validator_address">
            <b-col cols="12">
              <span class="font-weight-bolder">From: <router-link :to="`../validators/${item.validator_address}`">{{
              item.validator_address }}</router-link></span>
            </b-col>
            <b-col cols="12">
              <b-table :items="item.entries" borderless class="mt-1" striped hover responsive="sm" stacked="sm">
      ...
      <b-card v-if="delegations">
        <b-card-header class="pt-0 pl-0 pr-0">
          <b-card-title>Delegations</b-card-title>
          <div>
            <b-button v-b-modal.operation-modal variant="primary" size="sm" class="mr-25"
              @click="setOperationModalType('Delegate')">
              <feather-icon icon="LogInIcon" class="d-md-none" /><small class="d-none d-md-block">Delegate</small>
            </b-button>
            <b-button v-if="delegations" v-b-modal.operation-modal variant="primary" size="sm"
              @click="setOperationModalType('Withdraw')">
              <feather-icon icon="ShareIcon" class="d-md-none" /><small class="d-none d-md-block"> Withdraw
                Rewards</small>
            </b-button>
          </div>
        </b-card-header>
        <b-card-body class="pl-0 pr-0">
          <b-table :sort-by="rewards" :sort-desc="true" :items="deleTable" foot-variant="light" head-variant='light'
            striped sticky-header borderless responsive hover stacked="sm">
            <!-- <template #cell(vaidator)="data"> -->
            <!--  {{ data }} -->
            <!-- </template> -->
            ...
            <template #cell(reward)="data">
              <b-badge variant='light-success' :to="`../validators/${data.value.address}`">
                {{ data.value }}
              </b-badge>
            </template>
            <template #cell(token)="data">
              <b-badge variant='light-primary' :to="`../validators/${data.value.address}`">
                <feather-icon icon="UserCheckIcon" />
                {{ data.value }}
              </b-badge>
            </template>
            <template #cell(validator)="data">
              <b-badge v-b-tooltip.bottom.hover :title="`${data.value.address}`" v-b-tooltip.bottom.hover.v-dark
                variant='primary' :to="`../validators/${data.value.address}`">
                {{ data.value.moniker }}
              </b-badge>
            </template>
            <template #cell(action)="data">
              ...
              <b-button-group size="sm">
                <b-button v-b-modal.operation-modal v-ripple.400="'rgba(113, 102, 240, 0.15)'"
                  v-b-tooltip.hover.top="'Delegate'" v-b-tooltip.hover.v-dark variant="outline-info"
                  @click="selectDelegation(data,'Delegate')">
                  <feather-icon icon="LogInIcon" />
                </b-button>
                <b-button v-b-modal.operation-modal v-ripple.400="'rgba(113, 102, 240, 0.15)'"
                  v-b-tooltip.hover.top="'Redelegate'" v-b-tooltip.hover.v-dark variant="outline-success"
                  @click="selectDelegation(data,'Redelegate')">
                  <feather-icon icon="ShuffleIcon" />
                </b-button>
                <b-button v-b-modal.operation-modal v-ripple.400="'rgba(113, 102, 240, 0.15)'"
                  v-b-tooltip.hover.top="'Unbond'" v-b-tooltip.hover.v-dark variant="outline-warning"
      ...
      <b-card v-if="account" title="Profile" class="text-trancate">
        <b-table-simple borderless striped hover responsive stacked="sm">
          ...
          <b-tbody
            v-else-if="account.type === 'cosmos-sdk/PeriodicVestingAccount' && account.value.base_vesting_account">
            <b-tr>
              <b-td>
                Account Type
              </b-td>
            ...
            <b-tr>
              ...
              <b-td> {{ account.value.base_vesting_account.base_account.account_number }} </b-td>
            </b-tr>
            <b-tr>
              <b-td> Sequence </b-td>
              <b-td> {{ account.value.base_vesting_account.base_account.sequence }} </b-td>
            </b-tr>
            <b-tr>
              <b-td> Public Key </b-td>
              <b-td>
                <object-field-component :tablefield="account.value.base_vesting_account.base_account.public_key" />
              </b-td>
            </b-tr>
            <b-tr>
              <b-td> Original Vesting </b-td>
              <b-td> {{ formatToken(account.value.base_vesting_account.original_vesting) }} </b-td>
            </b-tr>
            <b-tr>
              <b-td> Delegated Free </b-td>
              <b-td> {{ formatToken(account.value.base_vesting_account.delegated_free) }} </b-td>
            </b-tr>
            <b-tr>
              <b-td> Delegated Vesting </b-td>
              <b-td> {{ formatToken(account.value.base_vesting_account.delegated_vesting) }} </b-td>
            </b-tr>
            <b-tr>
              <b-td> Vesting Time </b-td>
              <b-td> {{ formatTime(account.value.start_time) }} - {{
              formatTime(account.value.base_vesting_account.end_time) }}</b-td>
            </b-tr>
            <b-tr>
              ...
              <b-td>
                <b-table-simple>
                  <th>Length</th>
                  <th>Amount</th>
                  <b-tr v-for="p, index in account.value.vesting_periods" :key="index">
                    <td><small>{{ p.length }} <br>{{ formatLength(p.length) }}</small> </td>
          ...
          <b-tbody
            v-else-if="account.type === 'cosmos-sdk/DelayedVestingAccount' && account.value.base_vesting_account">
            <b-tr>
              <b-td>
                Account Type
              </b-td>
            ...
            <b-tr>
              <b-td> End Time </b-td>
              <b-td> {{ formatTime(account.value.base_vesting_account.end_time) }}</b-td>
            </b-tr>
          </b-tbody>
      ...
      <b-popover target="address-qr" variant="dark" triggers="hover" placement="bottom">
        <vue-qr :text="address" />
      </b-popover>
