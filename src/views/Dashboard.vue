<template>
  <div>
    ...
    <dashboard-price-chart-2 />
    <!-- Stats Card Vertical -->
    <b-row class="match-height">
      <b-col xl="2" md="4" sm="6">
        <dashboard-card-vertical icon="BoxIcon" :statistic="height" statistic-title="Height" />
      </b-col>
      <b-col xl="2" md="4" sm="6">
        <dashboard-card-vertical color="info" icon="DollarSignIcon" :statistic="supply"
      ...
      <b-col xl="2" md="4" sm="6">
        <dashboard-card-vertical color="danger" icon="UserCheckIcon" :statistic="validators"
    ...
    <b-card no-body>
      <b-card-header>
        ...
        <b-badge v-if="address" :to="`./${chain}/gov`" variant="light-success">
          <feather-icon icon="NavigationIcon" size="12" />
          Browse All
        </b-badge>
      </b-card-header>
      <b-card-body>
        <b-row v-for="prop in proprosals2" :key="prop.id">
          ...
          <b-col md="6" sm="12">
            <b-row>
              <b-col cols="8">
                <div class="scale">
                  <div class="box">
                    <b-progress :max="totalPower? 100 * (totalPower/prop.tally.total) :100" height="2rem" show-progress
                      class="font-small-1">
                  ...
                  <div
                    v-b-tooltip.hover
                    title="Threshold"
                  class="box overlay" :style="`left:${scaleWidth(prop)}%;`" />
          ...
          <b-col cols="12" :class="detailId === prop.id? 'd-block': 'd-none'">
            <b-card bg-variant="transparent" class="shadow-none" style="max-height:350px;overflow: auto;">
    ...
    <div style="justify-content: between; margin-top: 10px; padding-left: 10px; padding-right: 10px;" class="d-flex">
      <h2 class="pr-1 flex-start">
        <span class="text-capitalize">Wallet</span>&nbsp;
      </h2>
      <div class="breadcrumb-wrapper flex-grow-1">

        <b-badge size="sm" variant="light-success">
        ...
        <!--       icon="DollarSignIcon" -->
        <!--       size="16" -->
        <!--       class="text-info align-text-middle" -->
        <!--     /> -->
        <!--   </b-breadcrumb-item> -->
        <!--   <b-breadcrumb-item -->
        <!--     to="`/wallet`" -->
      ...
      <b-link v-if="address" :to="`./${chain}/account/${address}/receive`" style="font-size: 10px; padding: 2px;"
        class="text-success" variant="link" size="sm">
        <feather-icon icon="DollarSignIcon" size="12" />
        Receive&nbsp;
      </b-link>
      <b-link v-if="address" :to="`./${chain}/account/${address}`" style="font-size: 10px; padding: 2px;"
        class="text-info" variant="link" size="sm">
        <feather-icon icon="UserIcon" size="12" />
        Your Address&nbsp;
      </b-link>
      <b-link v-if="address" :to="`/wallet/settings`" style="font-size: 10px; padding: 2px;" class="text-warning"
        variant="link" size="sm">
        <feather-icon icon="SettingsIcon" size="12" />
        Settings&nbsp;
      </b-link>
      <b-badge variant="light-info" v-else :to="`/wallet/accounts`">
        Not connected?
      </b-badge>
    </div>
    <div style="margin-top: 30px; background: transparent; border: none; box-shadow: none;">
      <b-row>
        <b-col lg="3" sm="6">
      ...
      <b-row v-if="address" class="mt-1">
        <b-col cols="4">
          <b-button v-b-modal.operation-modal block variant="info"
            style="text-shadow: 1px 1px 3px 1px rgba(0,0,0,0.9) !important; font-weight-600;" @click="selectSend()">
            <feather-icon icon="SendIcon" />
            Send
          </b-button>
        </b-col>
        <b-col cols="4">
          <b-button v-b-modal.operation-modal block class="text-white" variant="success"
            style="!important; font-weight-600;" @click="selectSend()" :to="`/wallet/import`">
            <feather-icon icon="PlusIcon" />
            Connect
          </b-button>
        </b-col>
        <b-col cols="4">
          <b-button block variant="warning" class="text-black"
            style="text-shadow: 1px 1px 3px 1px rgba(0,0,0,0.9) !important; font-weight-600;"
      ...
      <b-row v-if="stakingList && stakingList.length > 0">
        <b-col>
          <b-card no-body>
            <b-table sort-by="delegation" footer-class="rounded-lg" header-class="rounded-lg"
              class="mb-0 h-100 rounded-xl" style="border-radius: 10px;" :items="stakingList" borderless
              header-variant="dark" footer-variant="dark" :fields="fields" striped hover responsive="sm" stacked="sm">
              <template #cell(rewards)="data">
                <b-badge variant='light-success' :to="`../validators/${data.value.address}`">
                  {{ data.value }}
                </b-badge>
              </template>
              <template #cell(delegation)="data">
              ...
              <template #cell(action)="data">
                <b-button-group class="d-none" size="sm">
            ...
            <!-- <b-card-footer class="text-right"> -->
              <!-- <b-badge>Total Delegations: {{ sum(delegations)}}</b-badge> -->
              <!-- <b-badge>Total Rewards: {{ sum(rewards)}}</b-badge> -->
            <!--   <b-button class="gradb text-white" variant="link" v-b-modal.operation-modal @click="selectWithdraw()"> -->
            <!--     <feather-icon icon="PlusIcon" /> -->
            <!--     Claim Rewards -->
            <!--   </b-button> -->
            <!-- </b-card-footer> -->
          ...
          <b-card>
            ...
            <b-card-body class="pl-0 pr-0">
              <b-row v-for="item in unbonding" :key="item.validator_address">
                ...
                <b-col cols="12">
                  <b-table-simple :items="item.entries" class="mt-1" striped hover responsive="sm" stacked="sm">
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'delegation', sortable: true },
        { key: 'rewards', sortable: true },
    ...
    stakingList() {
      return this.delegations.map(x => {
        ...
        return {
          ...
          validator: {
            moniker: getStakingValidatorOperator(this.$http.config.chain_name, x.delegation.validator_address, 8),
            address: x.delegation.validator_address,
          },
