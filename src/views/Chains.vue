<template>
  <div class="back text-center container-lg">
    <b-link>
      <div class="d-flex justify-content-center align-items-center">
        <!-- <vuexy-logo /> -->
        <h1 class="text-primary display-4 font-weight-bolder d-none d-md-block">
          <br />
          <span class="ollo font-family-monospace" style="font-weight: 800;">Explore Wisely</span><small
            class="text-muted flow-left">OLLO
            Station</small>
        </h1>
      </div>
    </b-link>
    <p class="mb-1 text-muted">
      <span class="ollo">OLLO explorer</span> aims to be an all-in-one explorer, wallet, and more.
    </p>
    <h2 class="mb-3 mt-3">
      OLLO Station
    </h2>
    <div>
      <b-row class="match-height justify-self-center justify-content-center">
        <b-col class="justify-self-center justify-content-center"
          v-observe-visibility="(visible) => visibilityChanged(visible, data)" sm="6" md="4" lg="4" xl="3">
          <router-link :to="ollo.chain_name">
            <b-card v-if="ollo" class="earnings-card text-left">
              <div>
                <b-card-title class="mb-1 d-flex justify-content-between">
                  <span class="text-uppercase">{{ ollo.chain_name }} <small class="font-small-2">Testnet {{
                  ollo.sdk_version }}</small></span>
                  <b-dropdown class="ml-1" variant="link" no-caret toggle-class="p-0" right>
                    ...
                    <b-dropdown-item :to="`/${ollo.chain_name}/`">
                      Overview
                    </b-dropdown-item>
                    <b-dropdown-item :to="`/${ollo.chain_name}/staking`">
                      Stake
                    </b-dropdown-item>
                    <b-dropdown-item :to="`/${ollo.chain_name}/gov`">
                      Governance
                    </b-dropdown-item>
                    <b-dropdown-item :to="`/${ollo.chain_name}/uptime`">
                      Uptime
                    </b-dropdown-item>
                    <b-dropdown-item :to="`/${ollo.chain_name}/statesync`">
                      Statesync
                    </b-dropdown-item>
                  </b-dropdown>
                </b-card-title>
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <div>
                    <b-avatar :src="ollo.logo" class="badge-minimal" variant="light-primary" rounded size="md" badge
                      :badge-variant="ollo.variant" />
                  </div>
                </div>
                <b-card-text class="text-muted font-small-2">
                  <span> Updated on </span><span class="font-weight-bolder">{{ ollo.time || '...' }}</span>
                </b-card-text>
              </div>
            </b-card>
          </router-link>
