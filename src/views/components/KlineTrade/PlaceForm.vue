  ...
  <div>
    <b-alert class="mt-2" variant="warning" show>
      ...
      <div class="alert-body">
        We're hard at work bringing top-of-the-line trading features to you all. Until then, trading is still under
        construction. Stay tuned!
      </div>
    </b-alert>

    <b-card bg-variant="light-primary" text-variant="black">
    ...
    <b-form-group>
      ...
      <b-input-group class="input-group-merge">
        <b-form-input id="amount" v-model="amount" type="number" placeholder="Quantity" @change="changeAmount()" />
        <b-input-group-append is-text>
          {{ base }}
        </b-input-group-append>
      </b-input-group>
      <b-alert variant="danger" :show="type === 1 && amount > available">
        <div class="alert-body">
    ...
    <b-form-group>
      <div class="d-flex justify-content-between">
        <span>Volume</span>
        <span>
          <small v-if="localPrice > 0" class="text-muted mr-1">≈${{ localPrice }}</small>
          <feather-icon id="popover-trading-setting" icon="SettingsIcon" class="text-primary" />
        </span>
      </div>
      <b-input-group class="input-group-merge">
        <b-form-input id="total" v-model="total" type="number" placeholder="Volume" @change="changeTotal()" />
        <b-input-group-append is-text>
          {{ target }}
        </b-input-group-append>
      </b-input-group>
      <b-alert variant="danger" :show="type === 0 && total > available">
        <div class="alert-body">
          Insufficient available amount
        </div>
      </b-alert>
    </b-form-group>
    <b-popover target="popover-trading-setting" placement="left" triggers="hover" boundary="scrollParent"
      boundary-padding="0" class="px-0">
      <b-form-group>
        <label for="slip">
          Slippage Tolerance
        </label>
        <div id="slip">
          <b-form-radio v-model="slippage" value="0.01" button button-variant="outline-secondary">
            1%
          </b-form-radio>
          <b-form-radio v-model="slippage" value="0.025" button block class="px-1" button-variant="outline-secondary">
            2.5%
          </b-form-radio>
          <b-form-radio v-model="slippage" value="0.05" button button-variant="outline-secondary">
            5%
          </b-form-radio>
        </div>
      </b-form-group>
    </b-popover>
    <b-form-group label="Signer" label-for="wallet">
      <b-form-radio-group v-model="wallet" inline>
        <b-form-radio v-model="wallet" name="wallet" value="keplr">
          <small>Keplr</small>
        </b-form-radio>
        <b-form-radio v-model="wallet" name="wallet" value="ledgerUSB">
          <small>Ledger(USB)</small>
        </b-form-radio>
        <b-form-radio v-model="wallet" name="wallet" value="ledgerBle" class="mr-0">
    ...
    <b-card-footer>
      <b-form-group>
        <b-button v-if="address" block :disabled="type === 0? total > available : amount > available"
          :variant="type === 0 ? 'success': 'danger'" @click="sendTx()">
          {{ type === 0 ? `Buy ${ base }` : `Sell ${ base }` }}
        </b-button>
        <b-button v-else to="/wallet/import" block variant="dark" class="gradb">
          Connect Wallet
        </b-button>

      </b-form-group>
    </b-card-footer>
    <b-alert variant="danger" :show="dismissCountDown" dismissible @dismissed="dismissCountDown=0"
      @dismiss-count-down="countDownChanged">
      <div class="alert-body">
        {{ error }}
      </div>
    </b-alert>
    <deposite-window :symbol="type === 0 ? target: base" :denom-trace="denomTrace[currentDenom]" />
...
export default {
  ...
  methods: {
    initialAddress() {
      const { chain } = this.$route.params || this.$store.state.config.ollo
      const accounts = getLocalAccounts()
      const current = this.$store.state.config.defaultWallet
