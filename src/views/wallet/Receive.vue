  ...
  <div class="d-flex justify-content-center">
    <b-card>
      ...
      <b-card-body class="d-flex justify-content-center flex-column">
        ...
        <div>
          <br />
          <router-link :to="`./wallet/accounts/${address}`">
            <small style="padding: 5px; color: muted">{{ address }}</small>
          </router-link>
          <feather-icon icon="CopyIcon" size="12" @click="copy()" />
        </div>
        <b-button v-if="fromAddress" v-b-modal.operation-modal block variant="success" class="mt-2">
          <feather-icon icon="SendIcon" />
          Go To Payment
        </b-button>
        <b-button v-else block class="mt-2" variant="primary" to="/wallet/import">
          Connect Wallet
        </b-button>
      </b-card-body>
    </b-card>
    <operation-modal :address="fromAddress" :to-address="address" type="Transfer" />
  </div>
</template>
