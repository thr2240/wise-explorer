<template>
  <div class="px-0">
    <b-tabs lazy class="group-tabs" pills small size="sm" style="font-size: 11px !important; padding: 1px;">
      <b-tab lazy pill title="Group By" class="group-tab" disabled></b-tab>
      <b-tab lazy pill title="Validator" class="group-tab" style="font-size: 11px !important; padding: 1px;">
        <b-card border-variant="primary">
          ...
          <b-card no-body style="border: none; padding: 2px;">
            <b-row style="border: none;">
              <b-col md="3">
                <h3>Height&nbsp;
                  <b-badge variant="light-primary" :to="`./blocks/${height}`" v-b-tooltip.hover.v-dark v-b-tooltip.hover
                    :title="`Go to block ${height}`">
                    {{ height }}
                  </b-badge>
                </h3>
                <small class="text-muted">
                  <feather-icon icon="ClockIcon" />&nbsp;Updated at {{ latestTime }}
                </small>
              </b-col>
              <b-col md="9">
                <b-input-group>
                  <b-input-group-prepend is-text>
                    <small>Only missed blocks&nbsp;</small>
                    <b-form-checkbox v-model="missedFilter" v-b-tooltip.hover v-b-tooltip.hover.v-dark
                      title="Only missed blocks" name="viewMissed" />
                  </b-input-group-prepend>
                  <b-form-input v-model="query" placeholder="Keywords to filter validators" />
                  <b-input-group-append>
                    <b-button to="./validators/uptime/my" variant="primary">
          ...
          <b-row style="margin-bottom: 5px;">
            <b-col v-for="(x,index) in uptime" :key="index" sm="12" md="4" xl="3" class="text-truncate">
              <div class="d-flex justify-content-between">
                <div>
                  <b-form-checkbox v-model="pinned" :value="`${chain}#${x.address}`"
                    class="custom-control-warning text-truncate" @change="pinValidator(`${chain}#${x.address}`)"
                    style="margin-bottom: 10px;">
                    <!-- <router-link :to="`./staking/${x.validator.address}`"> -->
                    <b-badge variant="light-primary">{{ index+1 }}</b-badge> {{ x.validator.moniker }}
                    <!-- </router-link> -->
                  </b-form-checkbox>
                </div>
                <span v-if="missing[x.address]">
                  <b-badge v-if="missing[x.address].missed_blocks_counter > 0" v-b-tooltip.hover.v-danger
                    variant="light-danger" :title="`${missing[x.address].missed_blocks_counter} missed blocks`"
                    class="text-danger text-bolder">
                    {{ missing[x.address].missed_blocks_counter }}
                  </b-badge>
                  <b-badge v-else v-b-tooltip.hover.v-success variant="light-success" title="Perfect! No missed blocks">
                    0
                  </b-badge>
                </span>
              </div>
                  ...
                  <div class="d-flex justify-content-between align-self-stretch flex-wrap">
                    <div v-for="(b,i) in blocks" :key="i" style="width:1.5%;">
                      <router-link :to="`./blocks/${b.height}`">
                        <div v-b-tooltip.hover.v-dark :title="b.height"
                          :class="b.sigs && b.sigs[x.address] ? b.sigs[x.address] : 'bg-light-success'" class="m-auto">
                          &nbsp;</div>
                      </router-link>
                    </div>
                  </div>
      ...
      <b-tab lazy pill title="Proposer" class="group-tap">
        <b-card border-variant="primary">
          <b-row>
            <b-col md="3" sm="12">
              <h3>Height&nbsp;
            ...
            <b-col md="3" sm="12">
              <b-input-group prepend="Fetch frequency">
                <b-form-select id="frequency" v-model="frequency" placeholder="Frequency to fetch data"
            ...
            <b-col sm="12" md="6">
              <b-input-group prepend="# of absent validators">
                <b-form-input id="threshold" v-model="threshold" placeholder="Absent validator number in blocks" />
              </b-input-group>
            </b-col>
          </b-row>
          <br />
          <b-row>
            <b-col v-for="(x, i) in proposerViewData" :key="i" md="4" sm="12" class="text-truncate">
              {{ x.name }}
              <b-badge v-if="x.proposed > 0" v-b-tooltip.hover.v-dark
