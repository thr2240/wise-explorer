<template>
  <div>
    ...
    <b-card v-for="(items, k) in groupedDelegations" border-variant="primary" :key="`row-${k}`" :title="k">
      <b-row class="px-0 bg-light-primary text-white">
        <b-col md="4" sm="12" class="p-1 font-weight-bold">
          VALIDATOR
        </b-col>
        <b-col md="3" sm="12" class="p-1 font-weight-bold">
          DELEGATION
        </b-col>
        <b-col md="5" sm="12" class="p-1 font-weight-bold">
          REWARD
        </b-col>
      </b-row>
      <b-row v-for="(row, j) in items" :key="`${row.validator.validator}-${j}`">
        <b-col md="4" sm="12" class="d-flex align-items-center">
          <b-badge variant="light-primary" :to="`/${row.validator.chain}/validators/${row.validator.validator}`">
            <div cols="6">
              <b-avatar :src="row.validator.logo" size="18" variant="light-primary" rounded="" />
              {{ row.validator.moniker }}
            </div>
          </b-badge>
        </b-col>
        <b-col md="3" sm="12" class="d-flex align-items-center">
          <b-badge variant="light-success" :to="`/${row.validator.chain}/validators/${row.validator.validator}`">
            {{ row.delegation }}
          </b-badge>
        </b-col>
        <b-col md="5" sm="12">
          <b-badge variant="light-info" :to="`/${row.validator.chain}/account/${row.delegator_address}`">
            {{ row.reward }}
