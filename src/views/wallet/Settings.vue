<template>
  <div>
    <form-wizard ref="wizard" color="#9167f0" style="background-color:transparent;" :title="null" :subtitle="null"
      shape="square" finish-button-text="Save" back-button-text="Previous" class="mb-3 md" @on-complete="formSubmitted">
      <!-- wallet tab -->
          ...
          <b-row class="justify-content-between">
            <b-col md="6">
              <b-card variant="secondary" :show="!accounts && device === 'keplr'">
                <b-form-group label="Select a device to connect" label-for="device">
            ...
            <b-col md="6">
              <b-card variant="secondary" :show="!accounts && device === 'keplr'">
                ...
                <div class="alert-body p-1">
                  <span>If Keplr has not enabled <code>{{ chainId }}</code>, enable it here.&nbsp;</span>
                  ...
                  <b-button variant="primary" @click="suggest()">
                    Enable Keplr
                  </b-button>
                </div>
              </b-card>
              ...
              <b-form-group label="Account Name" label-for="account_name">
                <validation-provider #default="{ errors }" name="Account Name" rules="required">
                  <b-form-input id="account_name" v-model="name" :state="errors.length > 0 ? false:null"
                    placeholder="Wallet Name" :readonly="edit" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
            </b-col>
              ...
              <b-form-group label="Derivate Address For Chains:">
                <validation-provider #default="{ errors }" name="addrs" rules="required">
                  <div class="demo-inline-spacing text-uppercase">
                    <b-row>
                      <b-col v-for="item, key in chains" :key="key" xs="12" md="4" lg="3" class="mb-25">
                        <b-form-checkbox v-model="selected" name="addrs" :value="key">
                          <b-avatar :src="item.logo" size="18" variant="light-primary" rounded="" />
                          <span v-b-tooltip.hover.v-primary :title="`Coin Type: ${item.coin_type}`"
