<template>
  <div class="px-0">
    <b-card>
      ...
      <b-row>
        <span v-if="uptime.length===0" class="text-danger"> Your node is out of active validator set</span>
        <b-col v-for="(x,index) in uptime" :key="index" sm="12" class="text-truncate">
          <div class="d-flex justify-content-between">
            <b-form-checkbox v-model="pinned" :value="`${chain}#${x.address}`" class="custom-control-warning"
              @change="pinValidator(`${chain}#${x.address}`)">
              <span class="d-inline-block text-truncate font-weight-bold align-bottom"> {{ x.validator.moniker }}
              </span>
            </b-form-checkbox>
          ...
          <div class="d-flex justify-content-between align-self-stretch flex-wrap">
            <div v-for="(b,i) in blocks" :key="i" style="width:1.5%;">
              <router-link :to="`./blocks/${b.height}`">
                <div v-b-tooltip.hover.v-second :title="b.height"
