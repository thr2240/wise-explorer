  ...
  <section>
    <b-card no-body>
      <b-card-header>
        <b-card-title>
          ...
          {{ proposal.title }}
          &nbsp;
          <b-badge v-b-tooltip.bottom.hover v-b-tooltip.bottom.hover.v-dark :to="`./gov/${proposal.id}`"
            :title="`Voting started ${formatDate(proposal.voting_start_time)} and ended ${formatDate(proposal.voting_end_time)}`"
            class="ml-50 text-right" :variant="`${statusCol(proposal.status)[1]}`"
            style="font-weight: 400; font-size: 12px;">
            <feather-icon :icon="`${statusCol(proposal.status)[2]}`" />
            {{ statusCol(proposal.status)[0] }}
          </b-badge>
        </b-card-title>
      </b-card-header>
      <b-card-body>
        ...
        <b-table-simple stacked="sm" hover striped>
          <tbody>
            ...
            <b-tr>
              ...
              <b-td>
                <b-badge variant="primary">
                  {{ formatToken(proposal.total_deposit) }}
                </b-badge>
              </b-td>
            </b-tr>
            <b-tr>
            ...
            <b-tr>
              ...
              <b-td>
                <b-badge variant="light-info">
                  {{ formatDate(proposal.voting_start_time) }} - {{ formatDate(proposal.voting_end_time) }}
                </b-badge>
              </b-td>
            </b-tr>
            <b-tr>
      ...
      <b-card-footer>
        ...
        <b-button v-b-modal.operation-modal :disabled="proposal.status!=2" variant="primary"
          class="btn float-right mg-2" @click="openModal('Vote')">
          Vote
        </b-button>
      </b-card-footer>
    </b-card>
    <b-card no-body>
      ...
      <b-card-body>
        <div>
          <div class="scale">
            <div class="box">
              <b-progress :max="totalPower && proposal.status ===2? 100 * (totalPower/proposal.tally.total) :100"
                height="2rem" class="mb-2" show-progress>
                ...
                <b-progress-bar :id="'vote-no'+proposal.id" variant="danger" :value="percent(proposal.tally.no)"
                  :label="`${percent(proposal.tally.no).toFixed()}%`" show-progress />
                <b-progress-bar :id="'vote-veto'+proposal.id" class="bg-danger bg-darken-4"
                  :value="percent(proposal.tally.veto)" :label="`${percent(proposal.tally.veto).toFixed()}%`"
                  show-progress />
                <b-progress-bar :id="'vote-abstain'+proposal.id" variant="secondary"
                  :value="percent(proposal.tally.abstain)" :label="`${percent(proposal.tally.abstain).toFixed()}%`"
                  show-progress />
              </b-progress>
              ...
              <div title="Threshold" v-if="tallyParam" class="box overlay" :style="`left:${scaleWidth(proposal)}%;`" />
            </div>
          </div>
    ...
    <b-card v-if="proposal.total_deposit" no-body>
      ...
      <b-card-footer>
        ...
        <b-button v-b-modal.operation-modal :disabled="proposal.status!=1" variant="primary"
          class="btn float-right mg-2" @click="openModal('GovDeposit')">
          Deposit
        </b-button>
        <b-button v-b-modal.operation-modal :disabled="proposal.status!=2" variant="primary"
