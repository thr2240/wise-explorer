<template>
  <b-tabs content-class="mt-1" pills lazy small>
    <!-- This tabs content will always be mounted -->
    <b-tab title="Active Proposals" pill lazy small>
      <b-row class="match-height">
        <b-col v-for="(p,i) in list" :key="`${p.id}-${i}`" lg="6" md="12">
          <b-card>
            <b-card-title @click="description(`model-${i}`)">
              <b-avatar v-b-tooltip.hover :src="p.chain.logo" variant="light-primary" size="22"
            ...
            <div class="overflow-auto" :class="descId === `model-${i}`?'d-block': 'd-none'">
              <object-field-component :tablefield="p.contents" :small="false" />
            </div>
            ...
            <b-progress :max="100" height="2rem" class="my-2" show-progress>
              <b-progress-bar :id="'vote-yes'+p.id" variant="success" :value="percent(p.tally.yes)" show-progress
                :label="`${percent(p.tally.yes).toFixed()}%`" />
              <b-progress-bar :id="'vote-no'+p.id" variant="danger" :value="percent(p.tally.no)"
                :label="`${percent(p.tally.no).toFixed()}%`" show-progress />
              <b-progress-bar :id="'vote-veto'+p.id" variant="danger" :value="percent(p.tally.veto)" class="bg-danger bg-darken-4"
                :label="`${percent(p.tally.veto).toFixed()}%`" show-progress />
              <b-progress-bar :id="'vote-abstain'+p.id" variant="info" :value="percent(p.tally.abstain)"
                :label="`${percent(p.tally.abstain).toFixed()}%`" show-progress />
            </b-progress>
            ...
            <b-card-footer class="pb-0">
              <span v-for="(v,k) in p.votes" :key="k">
                <b-badge :variant="color(v.vote.option)">{{ v.keyname }} : {{ formatOption(v.vote.option) }}</b-badge>
              </span>
            </b-card-footer>
          </b-card>
        </b-col>
    ...
    <b-tab title="Upgrade Events" lazy small pill>
...
import WalletUpgradeEvents from './Upgrades.vue'
