<template>
  <div class="text-center">
    <b-card v-if="calculateTotal !== '0'" border-variant="primary">
      <b-row class="mx-0 d-flex align-items-center">
        <b-col md="5" class="py-0">
          ...
          <!-- chart -->
          <chart-component-doughnut :height="160" :width="160" :data="calculateChartDoughnut" />
        </b-col>
        <b-col style="min-height: 100%; border-left: 1px solid rgba(255,255,255,0.07);" md="7">
          <h3>Wallet Accounts</h3>
          <chartjs-scatter :data="scatters" :height="160" :width="240" />
          <!-- <echart-scatter -->
          <!--   :items.sync="scatters" -->
          <!--   auto-resize -->
          <!-- /> -->
    ...
    <div v-for="item,index in accounts" :key="index">
      <div>
        <div class="d-flex justify-content-between align-items-end mb-1">
          <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" class="gradb" variant="link"
            :to="`/wallet/import?name=${item.name}`">
            <feather-icon icon="EditIcon" class="mr-50" />
            <span class="align-middle">{{ item.name }}</span>
          </b-button>
          <div class="mr-50">
            <b-button :to="`/wallet/import?name=${item.name}`" variant="outline-info" size="sm"
              class="mr-1 d-none d-md-inline-block">
              <feather-icon icon="EditIcon" />
              <span class="align-middle">Edit</span>
            </b-button>
            <b-button variant="outline-danger" size="sm" @click="disconnect(item.name)">
        ...
        <b-row>
          <b-col v-for="acc, j in item.address" :key="j" sm="12" md="6" xl="4">
            ...
            <b-card no-body class="card-browser-states">
              <b-card-header>
                ...
                <feather-icon v-b-tooltip.hover.v-danger :title="`Remove ${acc.chain.toUpperCase()}?`"
                  icon="XSquareIcon" size="18" class="cursor-pointer text-danger" @click="removeAddress(acc.addr)" />
              </b-card-header>
              <b-card-body class="text-truncate">
                <b-row>
                  <b-col>
                    <div class="d-flex justify-content-between">
                      <b-avatar :src="acc.logo" size="28" variant="light-primary" rounded />
                      <div class="text-right">
                        <h4 class="my-0">{{ currency }}{{ formatBalance(acc.addr) }}
                        </h4>
                        <small :class="formatBalanceChangesColor(acc.addr)"> {{ formatBalanceChanges(acc.addr)
                        }}</small>
                      </div>
                    </div>
                    <app-collapse>
                      <app-collapse-item title="Assets">
                        <template #header>
                          <div>
                            <feather-icon icon="CopyIcon" @click="copy(acc.addr)" />&nbsp;
                            <small class="text-muted">{{ formatAddr(acc.addr) }}</small>
                          </div>
                        </template>
                        <div v-for="b,i in balances[acc.addr]" :key="i" class="d-flex justify-content-between">
                          <div
                            class="ml-25 font-weight-bolder text-capitalize text-success d-flex flex-column text-left"
                            title="Balance">
                            {{ formatAmount(b.amount, b.denom) }} {{ formatDenom(b.denom) }}
                            <span class="font-small-2 text-muted text-nowrap">{{ currency }}{{ formatCurrency(b.amount,
                            b.denom) }}</span>
                          </div>
                          <div class="d-flex flex-column text-right">
                            <span class="font-weight-bold mb-0">{{ currency }}{{ formatPrice(b.denom) }}</span>
                            <small :class="priceColor(b.denom)" class="py-0">{{ formatChanges(b.denom) }}</small>
                          </div>
                        </div>
                        <div v-for="b,i in delegations[acc.addr]" :key="`d-${i}`"
                          class="d-flex justify-content-between align-items-center">
                          <div
                            class="ml-25 font-weight-bolder text-capitalize text-primary d-flex flex-column text-left"
                            title="Balance">
                            {{ formatAmount(b.amount, b.denom) }} {{ formatDenom(b.denom) }}
                            <span class="font-small-2 text-muted text-nowrap">{{ currency }}{{ formatCurrency(b.amount,
                            b.denom) }}</span>
                        ...
                        <b-button block size="sm" variant="outline-primary" :to="`/${acc.chain}/account/${acc.addr}`"
                          class="mt-1 mb-0" @click="updateDefaultWallet(item.name)">
    ...
    <operation-modal :type="operationModalType" :address="selectedAddress" :selected-chain-name="selectedChainName" />
  </div>
</template>
...
import ChartjsScatter from './components/charts/ChartjsScatter.vue'
